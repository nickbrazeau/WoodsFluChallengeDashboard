...
- **Agent Maestro**: Coordinates work across all agents, manages task prioritization, and ensures consistency
  - Workflow: Reviews project state → Delegates tasks → Monitors progress → Integrates outputs
...
- **Agent Web Code Reviewer**: Reviews web code for bugs, discrepancies, and best practices from agent Web Builder and Agent Web Designer 
  ...
  - Workflow: Receives code → Checks syntax/logic → Validates against standards → Provides feedback
...
- **Agent Web Designer**: Focused on the aesthetic appeal of the website and dashboard using Duke University's official color scheme
  - Workflow: Reviews UX requirements → Creates design mockups using Duke colors → Defines layouts following Duke branding → Iterates based on feedback
...
- **Agent Web Builder**: Builds website from Agent Summarizer outputs
  - Workflow: Receives documentation → Designs interface → Implements interactive elements → Deploys site

- **Agent Data Munger**: Works through all metadata, including excel workbooks (multiple sheets), powerpoints, scattered emails to understand what experiments, assays, and data have been generated from the various challenge studies
  - Workflow: Extracts data from multiple sources → Standardizes formats → Validates integrity → Outputs structured datasets
  - **Key Field Mappings**:
    ...
    - **Storage unit** = Sample location details (freezer → shelf → rack → box → row/col)
    - **Label path** = Complete storage location path

- **Agent Librarian**: Searches, catalogs, and analyzes scientific publications that have used data from the Duke influenza challenge studies. **CRITICAL PHASE 2 AGENT** - Operates after Data Munger to identify what data has been generated by collaborators before summarization.
  - Workflow: Queries PubMed/databases → Identifies relevant publications → **Extracts methods/results sections from PDFs** → Links citations to specific studies and samples → Analyzes data usage → Generates publication bibliography with data linkages → Passes to Human Curator
...
- **Agent Human Curator**: Facilitates human-in-the-loop review and manual content incorporation between data munging and interpretation phases
  - Workflow: Receives munged data + citation database from Librarian → Presents for human review → Accepts feedback and corrections → Incorporates manually provided tables/figures → Validates publication-to-sample linkages → Passes curated content to Agent Summarizer
  - **Key Responsibilities**:
...
- **Agent Data Linker**: Creates comprehensive linkages between publications, assay datasets, and sample inventory to establish complete data provenance chains
  - Workflow: Receives harmonized inventory + publication database + assay tracking → Links publications to specific assay datasets → Maps which samples were used in which publications → Creates sample-to-assay-to-publication provenance chains → Identifies data reuse patterns → Generates linkage reports → Passes to Validator
  - **Key Responsibilities**:
    - Link each publication to the specific assays that generated data (e.g., "Smith 2015 used RNA-seq data from samples X, Y, Z")
    - Create bidirectional mappings: Sample → Assays → Publications AND Publication → Assays → Samples
    ...
    - Build provenance graphs showing data flow from collection → assay → publication
...
- **Agent Validator**: Validates data consistency, integrity, and completeness across all processed datasets before human curation
  - Workflow: Receives linked data from Data Linker → Runs validation checks → Identifies data conflicts and inconsistencies → Flags orphaned records → Generates validation report with warnings and errors → Passes clean dataset + validation report to Human Curator
  - **Key Responsibilities**:
...
- **Agent Data Locator**: Searches for missing files and documents data file locations to prepare comprehensive data access documentation
  - Workflow: Analyzes gaps document → Searches filesystem for missing inventory files → Scans for sequencing data directories → Identifies assay data file locations → Creates file manifests → Documents what exists and what's missing → Generates data access guide → Passes to Human Curator
  - **Key Responsibilities**:
...
- **Agent Illustrator**: Generates publication-quality figures and visualizations from data, working with human-provided sketches and coordinating with all other agents
  - Workflow: Receives human sketch/wireframe → Interprets visualization requirements → Coordinates with Data Linker/Validator to access needed data → Generates figure using appropriate plotting libraries → Iterates based on human feedback → Produces final publication-ready figures → Saves to curated/figures/
  - **Key Responsibilities**:
    ...
    - Produce sample tracking sankey diagrams (collection → storage → transfer → publication)
...
- **Agent Summarizer**: Summarizes and understands the connections between all of the data that has been generated from the influenza studies across collaborators and where that data is currently located
  - Workflow: Ingests curated data (samples + citations + data usage) + validation reports + data location manifest → Identifies relationships → Creates summary reports → Documents data provenance → Generates narrative summaries → Passes comprehensive summary to Web Builder
...
1. **Extract and harmonize data** (Data Munger)
   - Extract data from Excel workbooks, metadata files
   - Standardize schemas and terminology
   - Normalize timepoints and participant IDs
   - Create unified data model
   - Output: Harmonized sample inventory

2. **Search and catalog publications** (Librarian)
   - Query PubMed for challenge study publications
   - Extract methods and results sections from PDFs
   - Link publications to specific studies
   - Identify collaborator-generated data and assays
   - Create publication database with data usage mappings
   - Output: Citation database with study linkages

3. **Process assay tracking data** (ImmuneProfiling Processing)
   - Extract assay data from tracking spreadsheets
   - Identify which samples have molecular profiling
   - Track RNA-seq, cytokines, metabolomics, wearables, etc.
   - Output: Assay tracking tables

4. **Link data across sources** (Data Linker)
...
5. **Validate data integrity** (Validator)
...
6. **Locate missing data files** (Data Locator)
   - Search for missing inventory files (DU19-03)
   - Identify sequencing data locations
   - Create file manifests
   - Document data access paths
   - Output: File locations and access guide

7. **Human Curation & Review** (Human Curator + Illustrator)
...
8. **Generate summaries** (Summarizer)
